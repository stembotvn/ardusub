...
const AP_Param::GroupInfo AP_AHRS::var_info[] = {
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("ROT_ROLL", 15, AP_AHRS, _rot_roll, 0),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("ROT_PIT", 16, AP_AHRS, _rot_pitch, 0),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("ROT_YAW", 17, AP_AHRS, _rot_yaw, 0),
...
void AP_AHRS::set_orientation()
{
    ...
    if (orientation != ROTATION_CUSTOM) {
        _ins.set_board_orientation(orientation);
        if (_compass != nullptr) {
            _compass->set_board_orientation(orientation);
        }
    } else {
        float xrad = DEG_TO_RAD * _rot_roll;
        float yrad = DEG_TO_RAD * _rot_pitch;
        float zrad = DEG_TO_RAD * _rot_yaw;
        ...
        _ins.set_board_orientation(orientation, &_custom_rotation);
        if (_compass != nullptr) {
